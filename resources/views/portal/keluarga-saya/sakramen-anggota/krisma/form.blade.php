...
@section('title', 'Edit Krisma — ' . $anggota->nama)
@section('content')
<div class="page-heading">
    <div class="page-title mb-3">
        <div class="row">
            <div class="col-12 col-md-6 order-md-1 order-last"><h3>Edit Krisma — {{ $anggota->nama }}</h3></div>
            <div class="col-12 col-md-6 order-md-2 order-first">
                <nav aria-label="breadcrumb" class="breadcrumb-header float-start float-lg-end">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item"><a href="{{ route('portal.sakramen-anggota.krisma', $anggota) }}">Krisma</a></li>
    ...
    <section class="section">
        <div class="card">
            <div class="card-header"><h5 class="card-title mb-0">Edit Krisma — {{ $anggota->nama }}</h5></div>
            <div class="card-body">
                @if ($errors->any())
                    <div class="alert alert-light-danger color-danger mb-3"><ul class="mb-0">@foreach ($errors->all() as $e)<li>{{ $e }}</li>@endforeach</ul></div>
                @endif
                <form action="{{ route('portal.sakramen-anggota.krisma.update', $anggota) }}" method="POST">
                    @csrf @method('PUT')
                    <div class="row g-3">
                        <div class="col-md-6">
                        ...
                        <div class="col-md-6">
                            <label class="form-label">Nama Krisma</label>
                            <input type="text" name="nama_krisma" class="form-control" value="{{ old('nama_krisma', $krisma?->nama_krisma) }}">
                        </div>
                        <div class="col-md-6">
                            <label class="form-label">Uskup Pemberi</label>
                            <select name="uskup_id" class="form-select">
                                <option value="">-- Pilih (opsional) --</option>
                                @foreach ($uskupList as $u)
                                    <option value="{{ $u->id }}" {{ old('uskup_id', $sakramen->klerus_id) == $u->id ? 'selected' : '' }}>{{ $u->nama }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                    <div class="d-flex gap-2 justify-content-end mt-4">
                        <a href="{{ route('portal.sakramen-anggota.krisma', $anggota) }}" class="btn btn-light-secondary">Batal</a>
