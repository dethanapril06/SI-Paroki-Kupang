...
@section('title', 'Edit Minyak Suci — ' . $anggota->nama)
@section('content')
<div class="page-heading">
    <div class="page-title mb-3">
        <div class="row">
            <div class="col-12 col-md-6 order-md-1 order-last"><h3>Edit Minyak Suci — {{ $anggota->nama }}</h3></div>
            <div class="col-12 col-md-6 order-md-2 order-first">
                <nav aria-label="breadcrumb" class="breadcrumb-header float-start float-lg-end">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item"><a href="{{ route('portal.sakramen-anggota.minyak-suci', $anggota) }}">Minyak Suci</a></li>
    ...
    <section class="section">
        <div class="row justify-content-center">
            <div class="col-12 col-lg-7">
                <div class="card">
                    <div class="card-header"><h5 class="card-title mb-0">Edit Minyak Suci — {{ $anggota->nama }}</h5></div>
                    <div class="card-body">
                        @if ($errors->any())
                            <div class="alert alert-light-danger color-danger mb-3"><ul class="mb-0">@foreach ($errors->all() as $e)<li>{{ $e }}</li>@endforeach</ul></div>
                        @endif
                        <form action="{{ route('portal.sakramen-anggota.minyak-suci.update', [$anggota, $sakramen]) }}" method="POST">
                            @csrf @method('PUT')
                            <div class="row g-3">
                                <div class="col-md-6">
                                ...
                                <div class="col-12">
                                    ...
                                    <input type="text" name="tempat_terima" class="form-control" value="{{ old('tempat_terima', $sakramen->minyakSuci?->tempat_terima) }}">
                                </div>
                                <div class="col-12">
                                    <label class="form-label">Nama Pemberi (manual)</label>
                                    <input type="text" name="nama_pemberi" class="form-control" value="{{ old('nama_pemberi', $sakramen->minyakSuci?->nama_pemberi) }}">
                                </div>
                                <div class="col-12">
                                    <label class="form-label">Keterangan / Sebab</label>
                                    <textarea name="keterangan_sebab" class="form-control" rows="2">{{ old('keterangan_sebab', $sakramen->minyakSuci?->keterangan_sebab) }}</textarea>
                                </div>
                            </div>
                            <div class="d-flex gap-2 justify-content-end mt-4">
                                <a href="{{ route('portal.sakramen-anggota.minyak-suci', $anggota) }}" class="btn btn-light-secondary">Batal</a>
