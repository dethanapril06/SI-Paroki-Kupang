...
@section('title', 'Ajukan Mutasi Umat KUB')
        ...
        <div class="page-title">
            <div class="row">
                <div class="col-12 col-md-6 order-md-1 order-last">
                    <h3>Ajukan Mutasi Umat KUB</h3>
                    <p class="text-subtitle text-muted">Permintaan perpindahan / mutasi untuk umat dalam KUB {{ $kub->nama }}.</p>
                </div>
                <div class="col-12 col-md-6 order-md-2 order-first">
                    <nav aria-label="breadcrumb" class="breadcrumb-header float-start float-lg-end">
                        <ol class="breadcrumb">
                            <li class="breadcrumb-item"><a href="{{ route('portal.mutasi.index') }}">Portal</a></li>
                            <li class="breadcrumb-item active">Ajukan Mutasi Umat KUB</li>
                        </ol>
                    </nav>
                </div>
            </div>
                ...
                <div class="col-12 col-lg-8">
                    ...
                    <div class="alert alert-light-primary color-primary mb-4">
                        <div class="d-flex gap-3 align-items-center">
                            ...
                            <div>
                                <strong>KUB {{ $kub->nama }}</strong>
                                <div class="small text-muted">
                                    Total Umat Aktif: {{ $umatList->count() }} orang &nbsp;|&nbsp;
                                    Anda bertindak selaku Ketua KUB
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="card">
                        <div class="card-header">
                            <h5 class="card-title">Form Request Mutasi Anggota KUB</h5>
                        </div>
                        <div class="card-body">
                            @if ($errors->any())
                            ...
                            <form action="{{ route('portal.mutasi.umat-kub.store') }}" method="POST">
                                @csrf

                                {{-- Pilih Umat Target --}}
                                <div class="mb-3">
                                    <label class="form-label fw-semibold">Pilih Umat yang Akan Dimutasi <span class="text-danger">*</span></label>
                                    <select name="umat_id" id="umat_id" class="form-select @error('umat_id') is-invalid @enderror" required>
                                        <option value="">-- Pilih Umat --</option>
                                        @foreach ($umatList as $u)
                                            <option value="{{ $u->id }}" {{ old('umat_id', request('umat_id')) == $u->id ? 'selected' : '' }}>
                                                {{ $u->nama }} (KK: {{ $u->keluarga?->kepalaKeluarga?->nama ?? '-' }} - Hubungan: {{ $u->hubungan_keluarga ?? '-' }})
                                            </option>
                                        @endforeach
                                    </select>
